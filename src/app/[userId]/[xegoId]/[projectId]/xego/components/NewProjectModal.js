...
import { fetchAllXegos, createProject } from "@/app/utils/api"
import { useRouter } from "next/navigation"

const NewProjectModal = ({ onClose, userId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Close modal and redirect to new project
      onClose()
      router.push(`/${userId}/${selectedXego._id}/${createdProject._id}/xego`)
    } catch (error) {
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {xegos.map((xego) => (
                    <div
                      key={xego._id}
                      onClick={() => !isSubmitting && setSelectedXego(xego)}
