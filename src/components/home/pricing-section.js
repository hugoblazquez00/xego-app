      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {plans.map((plan, index) => (
            <Card
              key={index}
              className={`border-border ${plan.popular ? "border-platinum-400 shadow-md relative" : ""}`}
            >
              {plan.popular && (
                <div className="absolute top-0 right-0 -translate-y-1/2 px-3 py-1 bg-platinum-500 text-white text-xs font-medium rounded-full">
                  Most Popular
                </div>
              )}
              <CardHeader>
                ...
                <ul className="space-y-3">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-center">
                      <Check className="h-5 w-5 text-platinum-400 mr-2 flex-shrink-0" />
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
              ...
              <CardFooter>
                <Button
                  className={`w-full ${plan.popular ? "bg-platinum-500 hover:bg-platinum-600 text-white" : "bg-transparent border border-platinum-300 hover:bg-platinum-100 text-foreground"}`}
                  asChild
                >
